...
class GetconController extends ResourceController {
  ...
  Future<Response> getContent() async {
    ...
    final  contents = await  contentQuery.fetch();//The fetch() execution method returns a List<Hero>.
  ...
  final contentQuery = Query<Question>(context)
    ..where((q) => q.id).equalTo(qid);    //相当于sql的SELECT id, name FROM _question WHERE id = #;语句

  final content = await contentQuery.fetchOne();//取一个//You can also fetch an object by its primary key with the method ManagedContext.fetchObjectWithID. 
  if (content == null) {
